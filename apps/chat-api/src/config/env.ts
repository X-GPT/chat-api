...
export const apiEnv = (() => {
	...
	invariant(
		...
		"DOCUMENT_GATEWAY_PUBLIC_URL is required",
	);
	...
	return {
		...
		// Must be reachable from inside the E2B sandbox. Trailing slash stripped.
		DOCUMENT_GATEWAY_PUBLIC_URL: Bun.env.DOCUMENT_GATEWAY_PUBLIC_URL.replace(
			/\/+$/,
			"",
		),
